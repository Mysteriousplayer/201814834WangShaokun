...
for i in range(len(doc_test)):
    ...
    for m in range(len(label)):
        if max_label==label[m]:
            index_isone+=1
    if index_isone!=1:
        label2[i]=1
        index_istwo+=1
        results_isone=map(sims_list.index, heapq.nlargest(1, sims_list))
        for result in results_isone:
            prediction=l_index[result]
    print(prediction)
...
print(accuary)
print(index_istwo/len(doc_test))
print(right_label2/index_istwo)